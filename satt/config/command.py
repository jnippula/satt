...
class SattConfig:
    ...
    def action(self):
        ...
        if self._variables['sat_os'] >= 0:
            ...
            print(('   Control bus   : ' + helper.color.BOLD + self._variables['sat_control_bus'] + helper.color.END), end=' ')
    ...
    def set_os(self):
        ...
        for osid in list(targetos.OsHelper.osdata.keys()):
        ...
        if selection.isdigit():
            if int(selection) in list(targetos.OsHelper.osdata.keys()):
                value = int(selection)
    ...
    def set_login_id(self):
        ...
        _login_id = input('   Give Login ID (%s): ' %(_temp_login_id, ))
        if _login_id != '':
    ...
    def set_control_ip(self):
        ...
        while correct is False:
            ...
            ip = input('   Give IP address to connect (current %s): ' %(self._variables['sat_control_ip'], ))
            if ip == '':
                break
            values = ip.split('.')
            if len(values) == 4:
            ...
            if idx > 0:
                if idx == 1:
                    ...
                    print((' ' + star + ' ' + str(idx) + ') ' + helper.color.BOLD +
                           targetos.OsHelper.osdata[setup['sat_os']].Name.ljust(max_key) + helper.color.END), end=' ')
                    print((': ' + helper.color.BOLD + setup['sat_trace_logging_method'] + helper.color.END +
                           '/' + helper.color.BOLD + setup['sat_control_bus'] + helper.color.END), end=' ')
